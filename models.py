...
from util import create_blobinfo_response
...
class BetaRelease(Download):
    ...
    mobileprovision = db.BlobProperty()
    # Wireless Manifest
    wmanifest = db.TextProperty()
    ...
    # Get wireless manifest
    def get_wireless_manifest(self):
        # There isn't already a manifest. Generate a new one.
        if self.wmanifest is None:
                        ...
                        'assets': [
                            {
                                ...
                                'url':               'https://mumble-ios.appspot.com/wdist/app.ipa',
                            },
                            {
                                'kind':              'display-image',
                                'needs-shine':        False,
                                'url':               'https://mumble-ios.appspot.com/static/app/icon.png', #57x57 app icon
                            },
                            {
                                'kind':              'full-size-image',
                                'url':               'https://mumble-ios.appspot.com/static/app/fullicon.png', # 512x512 app icon
                            },
                        ],
                        'metadata': {
                                'kind':               'metadata',
                                'bundle-identifier':  self.identifier,
                                'bundle-version':     self.version,
                                'kind':               'software',
                                'subtitle':           'Mikkel Krautz',
                                'title':              'Mumble iOS Beta',
                        },
                    },
                ]
            }, buf)
            self.wmanifest = buf.getvalue()
            self.save()
        return self.wmanifest
    
    # Create a Werkzeug response object that transfers this BetaRelease
    def get_blobinfo_response(self):
        return create_blobinfo_response(blobstore.BlobInfo.get(self.blobkey),
                    filename=self.filename, mimetype='application/x-ios-application')

    # Create a Werkzeug response object that transfers the BetaRelease with 'filename'
    @classmethod
    def get_blobinfo_response_for_fn(cls, filename):
        query = BetaRelease.all()
        query.filter('filename =', filename)
        release = query.get()
        if not release:
            Response('', mimetype='text/plain')
        return create_blobinfo_response(blobstore.BlobInfo.get(release.blobkey),
